...
class XGBoostForecaster:
    ...
    def train(self, data, product_id=None, lookback=7, verbose=False):
        """
        Train the model (wrapper for fit method for test compatibility)
        ...
        Returns:
            dict with training metrics including accuracy
        """
        # Convert DataFrame to list of dicts if needed
        if hasattr(data, 'to_dict'):
            data = data.to_dict('records')
        
        result = self.fit(data, lookback=lookback, verbose=verbose)
        
        # Add accuracy metric expected by test
        if 'mae' in result:
            error_margin = 15
            # Estimate accuracy from MAE (better MAE = higher accuracy)
    ...
    def forecast(self, days=30, product_id=None):
        ...
        if isinstance(predictions_list, list) and len(predictions_list) > 0:
            if isinstance(predictions_list[0], dict):
                return np.array([p['predicted'] for p in predictions_list])
